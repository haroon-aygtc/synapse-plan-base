...
export class PromptTemplateController {
  ...
  async findAll(
    ...
    const templates = await this.promptTemplateService.findAll(req.user.organizationId, {
      userId,
      includeInactive,
      category,
      isPublic,
  ...
  async findOne(
    ...
    const template = await this.promptTemplateService.findOne(id, req.user.organizationId, {
      includeChildren,
      includeAgents,
    });
    return plainToClass(PromptTemplateResponseDto, template, {
  ...
  async remove(@Param('id', ParseUUIDPipe) id: string, @Request() req: any): Promise<void> {
    await this.promptTemplateService.remove(id, req.user.sub, req.user.organizationId);
  }
  ...
  async validateTemplate(
    ...
    return this.promptTemplateService.validateTemplate(body.content, body.variables);
  }
