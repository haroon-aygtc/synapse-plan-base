...
export class AgentService {
  ...
  async execute(
    ...
    const execution = this.agentExecutionRepository.create({
      id: executionId,
      agentId: id,
      sessionId: executeDto.sessionId || uuidv4(),
    ...
    try {
      ...
      let sessionContext: ISessionContext | null = null;
      ...
      if (
        ...
        executeDto.includeToolCalls !== false
      ) {
      ...
      if (
        ...
        executeDto.includeKnowledgeSearch !== false
      ) {
        // Emit knowledge search event
        this.eventEmitter.emit(AgentEventType.AGENT_KNOWLEDGE_SEARCH, {
          executionId,
          agentId: id,
          query: executeDto.input,
          knowledgeSources: agent.knowledgeSources,
          timestamp: new Date(),
        });

        // In a real implementation, you would integrate with a knowledge service
        // For now, we'll just log the search
        this.logger.debug(
          `Knowledge search requested for agent ${id} with query: ${executeDto.input}`,
        );
      }
      ...
      if (executeDto.sessionId && sessionContext) {
        await this.sessionService.updateSession(executeDto.sessionId, {
          context: {
            conversationHistory: [
              ...conversationHistory,
              {
      ...
      this.logger.log(
        `Agent execution completed: ${executionId} for agent ${id} (${executionTime}ms)`,
      );
      ...
      this.logger.error(
        `Agent execution failed: ${executionId} for agent ${id}`,
        error,
      );
