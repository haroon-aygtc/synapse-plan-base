...
export class WidgetCommunicationService {
  ...
  public async establishChannel(
    ...
    if (
      !this.securityService.validateOrigin(origin, widget.configuration.security.allowedDomains)
    ) {
  ...
  public async processMessage(channelId: string, message: CrossOriginMessage): Promise<any> {
    const channel = this.communicationChannels.get(channelId);
    if (!channel?.isActive) {
      throw new Error(`Channel ${channelId} not found or inactive`);
    }
    ...
    try {
      ...
      if (handler.rateLimited) {
        const isAllowed = await this.securityService.checkRateLimit(channelId, message.action);
        if (!isAllowed) {
  ...
  public getActiveChannels(): CommunicationChannel[] {
    return Array.from(this.communicationChannels.values()).filter((channel) => channel.isActive);
  }

  // Default message handlers
  private async handlePing(message: CrossOriginMessage, context: MessageContext): Promise<any> {
    return {
      pong: true,
      timestamp: new Date(),
  ...
  private processMessageQueues(): void {
    for (const [channelId, queue] of this.messageQueues.entries()) {
      ...
      const maxMessagesToProcess = Math.floor((timeSinceLastProcess / 1000) * messagesPerSecond);
