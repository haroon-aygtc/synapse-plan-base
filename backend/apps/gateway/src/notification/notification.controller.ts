...
export class NotificationController {
  ...
  async getTemplates(
    ...
    try {
      const templates = await this.notificationService.getTemplates(req.user.organizationId, {
        category,
        type,
      });
  ...
  async getDeliveryStats(@Request() req: any) {
    try {
      ...
      const stats = await this.notificationService.getNotificationStats(req.user.organizationId);
