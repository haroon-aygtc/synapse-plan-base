...
export function AIAssistedConfiguration() {
  ...
  const { generateAgentConfig, isGenerating, previewAgentResponse } =
    useAIAssistant();

  const [description, setDescription] = useState("");
  const [selectedUseCase, setSelectedUseCase] = useState("");
  const [personalityTraits, setPersonalityTraits] =
    useState<PersonalityTrait[]>(PERSONALITY_TRAITS);
  ...
  const handlePersonalityChange = useCallback(
    (traitName: string, value: number[]) => {
      setPersonalityTraits((prev) =>
        prev.map((trait) =>
          trait.name === traitName ? { ...trait, value: value[0] } : trait,
        ),
      );
    },
    [],
  ...
  const generateConfiguration = useCallback(async () => {
    ...
    try {
      const config = await generateAgentConfig({
        ...
        personalityTraits: personalityTraits.reduce(
          (acc, trait) => ({
            ...acc,
      ...
      toast({
        ...
        description:
          "AI has generated your agent configuration. Review and adjust as needed.",
      });
    } catch (error) {
      toast({
  ...
  const generatePreview = useCallback(async () => {
    ...
    try {
      ...
      setPreviewResponse(
        "Preview unavailable. Please check your configuration.",
      );
    } finally {
              ...
              {personalityTraits.map((trait) => (
                <div key={trait.name} className="space-y-2">
                  ...
                  <Slider
                    value={[trait.value]}
                    onValueChange={(value) =>
                      handlePersonalityChange(trait.name, value)
                    }
                    max={100}
                    step={5}
                    className="w-full"
